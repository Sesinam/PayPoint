...
document.getElementById('quantity').addEventListener('input', 
    function Cost() {
        var unitPrice = [parseInt(document.getElementById('first').title), parseInt(document.getElementById('second').title),
        parseInt(document.getElementById('third').title), parseInt(document.getElementById('fourth').title), 
        parseInt(document.getElementById('fifth').title), parseInt(document.getElementById('sixth').title)];

        var Price = [15, 20];
        var quantity = document.getElementById('quantity').value;
        for(i=0; i<=unitPrice.length; i++) {
            console.log(unitPrice[i]);
            if(unitPrice[i] == Price[0]) {
                document.getElementById('totalCharge').innerHTML = "Your total charge is GHC" + multiply(quantity, Price[0]);
            }else if(unitPrice[i] == Price[1]) {
                document.getElementById('totalCharge').innerHTML = "Your total charge is GHC" + multiply(quantity, Price[1]);
            }
        }
    }
);
